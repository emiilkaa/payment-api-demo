...
begin
    ...
    alter table payment_api_app.request validate constraint request_old_records;
    commit;

    alter table payment_api_app.request rename to request_old;
    ...
    create table if not exists payment_api_app.request
        (
            ...
            TERMINAL_ID      VARCHAR(01) not null,
            MESSAGE          VARCHAR(256),
            EXTENSION_FIELDS JSONB,
            PAYMENT_ID       NUMERIC not null
        ) partition by range (date_created);

    create table if not exists payment_api_app.request_20240501(like payment_api_app.request including all);
    ...
    alter table payment_api_app.request_old drop constraint request_old_records;
    commit;
end
$BODY$;
